...
{
    public static function soapRequest($cert, $keyPEM, $token, $rfc, $rfcReceptor, $fechaInicial, $fechaFinal, $TipoSolicitud = 'CFDI')
    {
        $xmlString = RequestXmlRequest::getSoapBody($cert, $keyPEM, $rfc, $rfcReceptor, $fechaInicial, $fechaFinal, $TipoSolicitud);
    ...
    public static function getSoapBody($cert, $keyPEM, $rfc, $rfcReceptor, $fechaInicial, $fechaFinal, $TipoSolicitud)
    {
        if (!empty($rfcReceptor)) {
            $cadena = 'RfcReceptor="' . $rfcReceptor . '"';
            $rfcsolicita = $rfcReceptor;
        } else if (!empty($rfc)) {
            $cadena = 'RfcEmisor="' . $rfc . '"';
            $rfcsolicita = $rfc;
        }
        $dataToHash = '<des:SolicitaDescarga xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx"><des:solicitud '.$cadena.' RfcSolicitante="'.$rfcsolicita.'" FechaInicial="'.$fechaInicial.'" FechaFinal="'.$fechaFinal.'" TipoSolicitud="'.$TipoSolicitud.'"></des:solicitud></des:SolicitaDescarga>';
